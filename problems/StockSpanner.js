...
The span of the stock's price in one day is the maximum number of consecutive days (starting from that day and going backward) for which the stock 
...
class StockSpanner {
    ...
    next(price) {
        ...
        while (this.stack.length > 0 && price >= this.stack[this.stack.length - 1][0]) {
            const [lastStackItemPrice, lastStockItemSpan] = this.stack[this.stack.length - 1];
            if (price >= lastStackItemPrice) {
                this.stack.pop();
                span += lastStockItemSpan;
            }
        }
...
class StockSpanner {
    ...
    next(price) {
        this.output[this.curIdx] = 1;
        
        if (this.stack.length !== 0 && price >= this.stack[this.stack.length - 1][0])  {
            while (this.stack.length > 0) {
                const [lastStackItemPrice, lastStockItemIdx] = this.stack[this.stack.length - 1];
                if (price >= lastStackItemPrice) {
                    this.stack.pop();
                    this.output[this.curIdx] += this.output[lastStockItemIdx];
                } else break;
            }
        }

        this.stack.push([price, this.curIdx]);
        const result = this.output[this.curIdx];
        this.curIdx++;

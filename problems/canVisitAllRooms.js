...
var canVisitAllRooms = function(rooms) {
    const seenArr = new Array(rooms.length).fill(false);
    seenArr[0] = true;
    ...
    while (stack.length > 0) {
        ...
        for (let canVisitRoom of rooms[curRoom]) {
            if (!seenArr[canVisitRoom]) {
                seenArr[canVisitRoom] = true;
                stack.push(canVisitRoom);
            }
        }
    }
    return seenArr.every((a) => !!a);
};

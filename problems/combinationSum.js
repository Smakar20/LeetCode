...
var combinationSum = function(candidates, target) {
    let output = [];
    combinationSumUtil(candidates, 0, target, output, []);
    return output;
};

function combinationSumUtil(candidates, curSum, target, output, curCombination) {
    if (curSum === target) {
        output.push([...curCombination]);
        return;
    }
    if (curSum > target) return;
    for (let i in candidates) {
        curCombination.push(candidates[i]);
        combinationSumUtil(candidates.slice(i), curSum + candidates[i], target, output, curCombination)
        curCombination.pop();
    }
}

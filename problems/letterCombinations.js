...
var letterCombinations = function(digits) {
    ...
    const findCombinations = (str, curStr) => {
        if (str.length === 0 && curStr.length > 0) {
            combinations.push(curStr);
            return;
        }

        const [first, remaining] = [str[0], str.slice(1)];

        for (let letter of digitLettersMap[first]) {
            findCombinations(remaining, curStr + letter);
        }
    };
